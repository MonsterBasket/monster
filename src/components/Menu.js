...
export default function MonsterMenu(){
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('pointerdown', e => handleClick(e))
      window.removeEventListener('scroll', e => {if(menuClose == "open") setMC("closed")})
    }
  ...
  function scrollTo(id){
    // if (path.pathname === "/" || path.pathname === "")
      ...
      // setTimeout(() => document.querySelector(`#home`).scrollIntoView(), 50)
  }
