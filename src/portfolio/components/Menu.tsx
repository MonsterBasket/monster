...
export default function Menu({scrollPos, buttonWidth, setButtonWidth, buttonOpacity, setButtonOpacity, setTurnToCheat}:Props){
  ...
  function turnToCheat(page:number){
    if (scrollPos < 3 * -window.innerHeight) setTurnToCheat(page)
  ...
  useBodyScrollPosition(({ prevPos, currPos }) => {
    ...
    let windowLimit: number = Math.max(window.innerHeight * 3, 2000);
    ...
    else if (currPos.y > -windowLimit) { // scrolled away from top
    ...
    if (currPos.y < 3 * -window.innerHeight){
      let farLeft: number = buttonWidth * 2 + 30; // distance from left of buttons to centre of buttons
      let middle: number = window.innerWidth / 2; // centre of screen width
      let leftAdjust: number = (currPos.y + window.innerHeight * 3) / -window.innerHeight * 2;
      leftAdjust = (1 - leftAdjust) * (middle - farLeft) + farLeft;
      if (currPos.y > 3.5 * -window.innerHeight){
        //slide to the left
  ...
  return <div className="menuCont">
    <div style={mainMenuStyle} id="menu">
      <a href="#About" onClick={() => turnToCheat(0)} style={aboutStyle} className="menuButton aboutButton">
      ...
      <a href="#Projects" onClick={() => turnToCheat(1)} style={projectsStyle} className="menuButton projectsButton">
      ...
      <a href="#Work" onClick={() => turnToCheat(2)} style={workStyle} className="menuButton workButton">
